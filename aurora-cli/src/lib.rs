...
fn extract_audio(video_filename: &str) {
    let extract_audio_cmd_str =
        format!("ffmpeg -i {video_filename} -vn -acodec copy {video_filename}_audio.m4a");
...
fn decode_frames(video_filename: &str, video_frames_dir_name: &str) {
    let decode_frames_cmd =
        format!("ffmpeg -i {video_filename} {video_frames_dir_name}/frame_%08d.png");
